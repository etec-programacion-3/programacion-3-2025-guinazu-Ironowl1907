...
class _ExercisePageState extends State<ExercisePage> {
  ...
  void initState() {
    super.initState();
    // Load exercises when the page initializes
  ...
  Future<void> _editExerciseDialog(
    ...
    // Find the muscle group by ID from the available list
    MuscleGroup? selectedMuscleGroup;
    ...
    if (result != null && mounted) {
      if (result['delete'] == true) {
        ...
        final Exercise updatedExercise = Exercise(
          id: exercise.id, // Keep the original ID for updates
          name: result['name'],
          description: result['description'],
          muscleGroupId: result['muscleGroupId'],
        );

        context.read<ExerciseProvider>().update(updatedExercise);
      }
    }
  }
  ...
  String _getMuscleGroupName(int? muscleGroupId) {
    ...
    final muscleGroups = context.read<MuscleGroupProvider>().muscleGroups;
